...
#
# Updates the single AGENT_WATCH_BUNDLE_ID build setting, from which the iPhone
...
ID="${1:-}"
if [ -z "$ID" ]; then
  echo "Usage: $0 <bundle-id>     e.g. $0 com.yourname.agentwatch" >&2
  exit 2
fi
...
if ! printf '%s' "$ID" | grep -qE '^[A-Za-z][A-Za-z0-9-]*(\.[A-Za-z0-9-]+)+$'; then
  echo "Error: '$ID' is not a valid reverse-DNS bundle id (e.g. com.yourname.agentwatch)." >&2
  exit 2
fi
case "$ID" in
...
# pbxproj: AGENT_WATCH_BUNDLE_ID = <val>;   (Debug + Release configs)
/usr/bin/sed -i '' -E "s/(AGENT_WATCH_BUNDLE_ID = )[^;]+;/\1${ID};/g" "$PBX"
# project.yml: AGENT_WATCH_BUNDLE_ID: <val>
[ -f "$YML" ] && /usr/bin/sed -i '' -E "s/(AGENT_WATCH_BUNDLE_ID: ).*/\1${ID}/" "$YML"
