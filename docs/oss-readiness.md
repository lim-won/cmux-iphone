...
```jsonc
{
  ...
  "pairing": {
    ...
    "fixedCode": null,        // set only when mode=="fixed" (WATCH_PAIR_CODE)
    "ttlMs": 86400000
  },
  "cmux": {
...
9. Fold `cmux-password` from `~/.config/claude-watch/` (`cmux.js:58`) into the single `dataDir` so all secrets live in one base dir.
...
```
1. git clone https://github.com/limseungwon/claude-watch
...
- Pairing code: rotating 6-digit + 24h TTL by default; fixed only via explicit `WATCH_PAIR_CODE`/config.
...
### Relevant files
- `/Users/limseungwon/claude-watch/skill/bridge/server.js` (`:69` pairing code, `:64-79,163-168` constants)
- `/Users/limseungwon/claude-watch/skill/setup-hooks.sh` (`:53-56,190-195` hook scope; `:227-284` codex-watch heredoc)
- `/Users/limseungwon/claude-watch/skill/bridge/run-in-cmux.sh` (`:7,12` personal path)
- `/Users/limseungwon/claude-watch/skill/bridge/cmux.js` (`:38,54-70` socket path + cruft)
- `/Users/limseungwon/claude-watch/skill/bridge/install-launchd.sh` (misleading cmux claim)
- `/Users/limseungwon/claude-watch/ios/ClaudeWatch/project.yml` (`com.shobhit.claudewatch` bundle ID)
- `/Users/limseungwon/claude-watch/skill/bridge/package.json` (no `bin` field; `bonjour-service` only)